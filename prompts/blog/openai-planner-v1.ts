...
- "{사이트명} 토토사이트 정보 리포트: 공개 제보, 도메인, DNS 조회 기준 정리" 형태는 fallback 용도로만 사용한다.
- Google 검색 노출 기준에 맞춰 각 페이지의 title은 고유하고 설명적이어야 한다. 사이트명만 바뀌고 나머지 문구가 동일한 boilerplate 제목 후보를 대량 생성하지 않는다.
- 제목 후보는 Source Snapshot의 seo_title_signals 중 사이트명 외 최소 2개 이상, 가능하면 2~3개를 조합한다: 리뷰 수, 공개 피해 제보 수, 추가 도메인 수, DNS 레코드 타입, WHOIS 등록일 여부, 네임서버 정보, IP 수, 최신 리뷰/제보 시각, DNS/WHOIS 조회 시각.
- 모든 사이트에 "{사이트명}"만 바뀌고 나머지 문구가 동일한 제목 후보를 반복하지 않는다.
- 제목 후보 생성 규칙:
  1. 공개 피해 제보가 1건 이상이면 "{사이트명} 공개 먹튀 피해 제보 {n}건 정리: 도메인·DNS·리뷰 현황" 방향을 사용한다. 단, "먹튀 확정", "먹튀 사이트", "위험 사이트"처럼 단정하지 않는다.
  2. 피해 제보는 없고 승인 리뷰가 있으면 "{사이트명} 토토사이트 후기 {n}건과 공개 먹튀 제보 0건: DNS 조회 기준" 방향을 사용한다. "먹튀 없음"이라고 쓰지 않는다.
  3. 추가 도메인이 여러 개면 "{사이트명} 도메인 {n}개 관측 현황: WHOIS·DNS·공개 제보 정리" 방향을 사용한다.
  4. WHOIS 등록일 정보가 있으면 "{사이트명} WHOIS 등록일·네임서버 정보: 공개 제보와 DNS 기록 정리" 방향을 사용한다.
  5. DNS 레코드 타입이 풍부하면 "{사이트명} DNS 레코드 분석: A·NS·TXT 기록과 공개 제보 현황" 방향을 사용한다.
  6. 정보가 부족하면 "{사이트명} 기본 정보 확인: 공개 먹튀 제보 0건·리뷰 0건·도메인 {domain_count}개" 방향을 fallback으로 사용하고, 억지로 강한 SEO 제목을 만들지 않는다.
- meta_title은 짧게 작성한다. 예: "{사이트명} 정보 | 도메인·DNS·공개 제보 현황", "{사이트명} 후기·제보 현황 | DNS 조회 기준", "{사이트명} 도메인 정보 | WHOIS·리뷰 데이터", "{사이트명} 공개 제보 현황 | 리뷰·DNS 정리".
- H1은 검색 title보다 조금 길어도 되지만, 동일한 boilerplate를 반복하지 않는다. 예: "{사이트명} 공개 데이터 리포트: 도메인 이력, DNS 기록, 승인 리뷰와 피해 제보 현황".
- 제목 후보에는 다음 표현을 사용하지 않는다: 추천, 안전놀이터, 먹튀 없음, 검증 완료, 가입, 보너스, 이벤트, 우회주소, 최신 접속주소.
- 먹튀 제보가 0건인 경우에도 "먹튀 없음"이라고 쓰지 말고, "공개 제보 현황" 또는 "공개 먹튀 제보 0건"처럼 표현한다.
...
- 피해 제보가 없으면 "조회 시점 기준 공개 승인된 피해 제보는 확인되지 않음"처럼 표현한다.
