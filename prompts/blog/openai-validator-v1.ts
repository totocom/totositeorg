...
- 실제 이용자 후기처럼 보이는 문장, 실제 먹튀 제보처럼 보이는 문장, 원문처럼 보이는 직접 인용문은 제거한다.
- 피해 금액, 발생일, 이용 기간, 고객센터 응답 여부는 Source Snapshot의 승인된 DB 데이터에 있는 경우에만 유지하고, 없으면 "확인되지 않음"으로 고친다.
- 후기와 제보는 반드시 "승인된 데이터 기준 요약"이라고 표현되도록 정리한다.
