...
test("formatObservationDescriptionForPublic separates long notice from description", () => {
  const siteSpecificNoticeText = buildSiteDescriptionNoticeText();
...
test("buildSiteDescriptionNoticeText renders static non-promo disclosure", () => {
  assert.equal(
    buildSiteDescriptionNoticeText(),
...
test("site description notice component is rendered above the overview description", () => {
  ...
  assert.match(
    pageSource,
    /<SiteDescriptionNotice\s*\/>[\s\S]*<SafeMarkdown/,
  );
