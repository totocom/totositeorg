...
create table if not exists public.site_crawl_snapshots (
  ...
  constraint site_crawl_snapshots_source_type_allowed check (
    source_type in ('manual_html', 'crawler')
  ),
  constraint site_crawl_snapshots_html_input_type_allowed check (
    html_input_type in ('source_html', 'rendered_html', 'unknown')
  ),
  constraint site_crawl_snapshots_status_allowed check (
    snapshot_status in ('draft', 'extracted', 'ai_generated', 'approved', 'rejected')
  ),
  constraint site_crawl_snapshots_source_url_format check (
    source_url is null
    or source_url ~* '^https?://'
  ),
  constraint site_crawl_snapshots_final_url_format check (
    final_url is null
    or final_url ~* '^https?://'
  ),
  constraint site_crawl_snapshots_json_shapes check (
...
create table if not exists public.blog_posts (
  ...
  featured_image_captured_at timestamptz null,
  site_logo_url text null,
  site_logo_alt text null,
  ...
  constraint blog_posts_featured_image_alt_not_blank check (
    featured_image_alt is null
    or length(trim(featured_image_alt)) > 0
  ),
  constraint blog_posts_featured_image_caption_not_blank check (
    featured_image_caption is null
    or length(trim(featured_image_caption)) > 0
  ),
  constraint blog_posts_site_logo_alt_not_blank check (
...
create table if not exists public.blog_source_snapshots (
  ...
  crawl_snapshot_json jsonb null,
  ...
  constraint blog_source_snapshots_crawl_snapshot_object check (
